...
const assertions = {
  ...
  'five recruiter role lenses rendered': root?.querySelectorAll('.role-lens-tabs button').length === 5,
  'role lens exposes three relevant projects': root?.querySelectorAll('.role-lens-projects a').length === 3,
  'profile image has alt text': root?.querySelector('.profile-portrait img')?.getAttribute('alt') === 'Raveesh Raj Grandhi',
  'professional impact is visible': root?.querySelectorAll('.profile-impact > div').length === 4,
  'skip link exists': Boolean(root?.querySelector('.skip-link')),
  'all primary sections are in main': root?.querySelectorAll('main > section').length === 8,
  'primary recruiter CTA opens contact': root?.querySelector('.nav-contact')?.getAttribute('href') === '#contact',
  'direct email fallback exists': root?.querySelector('.contact-form-foot a[href^="mailto:"]')?.getAttribute('href') === 'mailto:raveeshraj26@gmail.com',
