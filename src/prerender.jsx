...
export const renderProjects=()=>renderToStaticMarkup(
  <main className="archive-page">
    ...
    <p>Explore work across data engineering, business intelligence, product analytics, and decision science.</p>
    <div className="archive-list">{archive.map(([n,title,field,result,repo])=>
      <a key={repo} href={'https://github.com/Raveesh-rajg/'+repo} target="_blank" rel="noreferrer"><span>{n}</span><strong>{title}</strong><span>{field}</span><span>{result}</span></a>
    )}</div>
  </main>
);
